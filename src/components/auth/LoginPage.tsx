...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-titan-bg relative overflow-hidden flex items-center justify-center p-4">
      {/* Background Effects */}
      <div className="fixed inset-0 pointer-events-none">
        ...
        />
        <div className="absolute top-[-200px] left-[-100px] w-[600px] h-[600px] rounded-full bg-titan-cyan/[0.06] blur-[150px]" />
        <div className="absolute bottom-[-200px] right-[-100px] w-[500px] h-[500px] rounded-full bg-titan-purple/[0.08] blur-[150px]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full bg-titan-magenta/[0.04] blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-5xl">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-titan-cyan/10 border border-titan-cyan/20 mb-4">
            <Zap className="w-3.5 h-3.5 text-titan-cyan" />
            <span className="font-mono text-xs text-titan-cyan">DEMO MODE — No Signup Required</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          ...
          <motion.div
            ...
            className="space-y-3"
          >
            <h2 className="font-display font-bold text-sm text-white/60 mb-3 flex items-center gap-2">
              ...
              {demoAccounts.map((account, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className="group relative glass-card p-4 text-left transition-all duration-300 hover:border-white/20 active:scale-[0.97] disabled:opacity-50"
                    style={{
                      boxShadow: isLoading ? `0 0 20px ${account.color}30` : undefined,
                      borderColor: isLoading ? `${account.color}40` : undefined,
                    }}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-display font-bold text-sm text-white group-hover:text-white/90">
                            {account.label}
                          </span>
                          {account.role === 'client' && (
              ...
              <div className="relative z-10">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-lg bg-gradient-to-r from-titan-cyan/20 to-titan-purple/20 border border-titan-cyan/30 text-white font-display font-bold text-sm hover:from-titan-cyan/30 hover:to-titan-purple/30 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2"
                  >
                    {loginLoading ? (
